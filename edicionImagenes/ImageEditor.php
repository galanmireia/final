...
class ImageEditor {
  ...
  function resize($width, $height) 
  {
    if(!$this->error) 
    {
     ...
     // imagecopyresized ($tmpimage, $this->img, 0, 0, 0, 0, $width, $height, $this->x, $this->y);
  ...
  ########################################################
  # crea thumbnails de tamaño x,y con un borde (todavia no)y un color de fondo 
  ########################################################
function thumbnail($width, $height,$rojo,$verde,$azul) 
{	if(!$this->error) 
	{	$intAnchoO=$this->x;
		$intAltoO=$this->y;
		$intAnchoN=$width;	//80
		$intAltoN=$height;		//80
		if ($intAltoN<$intAltoO || $intAnchoN<$intAnchoO)
		{	if ($intAnchoO/$intAnchoN>$intAltoO/$intAltoN)
			{	$intAnchoD=$intAnchoN;
				$intAltoD=$intAltoO/($intAnchoO/$intAnchoN);
				$intAjustar=1;	//alto
			}
			else
			{	$intAltoD=$intAltoN;
				$intAnchoD=$intAnchoO/($intAltoO/$intAltoN);
				echo "alto d:".$intAltoD."<br>";
				echo "ancho d:".$intAnchoD."<br>";
				echo "ancho o:".$intAnchoO."<br>";
				echo "alto o:".$intAltoO."<br>";
				echo "alto n:".$intAltoN."<br>";
				$intAjustar=0;	//ancho
			}
		}
		else
		...
		$tmpimage = imagecreatetruecolor($intAnchoN, $intAltoN);
		$margenx= ($intAnchoN-$intAnchoD)/2;
		$margeny= ($intAltoN-$intAltoD)/2;
		$colour = ImageColorAllocate($tmpimage, $rojo,$verde,$azul);
		ImageFill($tmpimage, 0, 0, $colour);
		//imagecopyresampled ( resource img_dst, resource img_org, int Xdst, int Ydst, int Xorg, int Yorg, int ancho_dst, int alto_dst, int ancho_org, int alto_org )
		//echo "ancho o:".$intAnchoO."<br>";
		//echo "ancho d:".$intAnchoD."<br>";
		//echo "alto o:".$intAltoO."<br>";
		//echo "alto d:".$intAltoD."<br>";
		imagecopyresampled($tmpimage, $this->img, $margenx, $margeny, 0, 0, $intAnchoD, $intAltoD, $intAnchoO, $intAltoO);
		//imagecopyresized ($tmpimage, $this->img, $margenx, $margeny, 0, 0, $intAnchoD, $intAltoD, $intAnchoO, $intAltoO);
		imagedestroy($this->img);
		$this->img = $tmpimage;
		$this->y = $height;
		$this->x = $width;
  ...
  function crop($x, $y, $width, $height) 
  {
    if(!$this->error) 
    {
      ...
      //imagecopyresized($tmpimage, $this->img, 0, 0, $x, $y, $width, $height, $width, $height);
